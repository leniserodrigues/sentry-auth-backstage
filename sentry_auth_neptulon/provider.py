...
from .views import FetchUser, BackstageConfigureView


class BackstageOAuth2Login(OAuth2Login):
    authorize_url = AUTHORIZE_URL
    client_id = CLIENT_ID
    scope = SCOPE

    def __init__(self):
        super(BackstageOAuth2Login, self).__init__()

    def get_authorize_params(self, state, redirect_uri):
        params = super(BackstageOAuth2Login, self).get_authorize_params(
            state, redirect_uri
        )
...
class BackstageOAuth2Provider(OAuth2Provider):
    name = 'backstage'
    client_id = CLIENT_ID
    client_secret = CLIENT_SECRET

    def __init__(self, **config):
        super(BackstageOAuth2Provider, self).__init__(**config)

    def get_configure_view(self):
        return BackstageConfigureView.as_view()

    def get_auth_pipeline(self):
        return [
            BackstageOAuth2Login(),
            OAuth2Callback(
